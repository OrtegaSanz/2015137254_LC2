...
namespace _2015137254_MVC.Controllers
{
    public class DireccionesController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            Direccion Direcciones = unityOfWork.Direcciones.Get(id.Value);
            if (Direcciones == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Create([Bind(Include = "DireccionId")] Direccion Direcciones)
        {
            if (ModelState.IsValid)
            {
                ...
                unityOfWork.Direcciones.Add(Direcciones);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "DireccionId")] Direccion Direcciones)
        {
            if (ModelState.IsValid)
            {
                //db.Entry(administradorEquipo).State = EntityState.Modified;
                unityOfWork.StateModified(Direcciones);
                //db.SaveChanges();
                unityOfWork.SaveChanges();
                return RedirectToAction("Index");
            }
        ...
        {
            //AdministradorEquipo administradorEquipo = db.AdministradorEquipos.Find(id);
            Direccion Direcciones = unityOfWork.Direcciones.Get(id);
            unityOfWork.Direcciones.Delete(Direcciones);
            //db.SaveChanges();
            unityOfWork.SaveChanges();
            return RedirectToAction("Index");
        }
